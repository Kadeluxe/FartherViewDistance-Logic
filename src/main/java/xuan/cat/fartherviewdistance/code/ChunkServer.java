...
//        }, 0, 20);
    ...
    private void tickSync() {
        lastOnlinePlayers = Bukkit.getOnlinePlayers();
...
//            branchPacket.sendViewDistance(view.getPlayer(), view.getMap().extendDistance);
    ...
    public void packetEvent(Player player, PacketEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Server.MAP_CHUNK) {
            int[] chunkLocation = branchPacket.readChunkLocation(event.getPacket());
            int chunkX = chunkLocation[0];
            int chunkZ = chunkLocation[1];
            view.send(chunkX, chunkZ);
        }
    }

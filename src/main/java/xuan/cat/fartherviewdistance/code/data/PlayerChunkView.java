...
//            mapView.markOutsideWait(newDistance);
    ...
    public int max() {
        ...
        } else if (configWorld.playerIndependentPermissions) {
            // 檢查權限節點
            for (int permissionViewDistance = 1 ; permissionViewDistance <= configWorld.maxViewDistance; permissionViewDistance++) {
                if (player.hasPermission("max_view_distance." + permissionViewDistance)) {
                    viewDistance = permissionViewDistance;
                }
            }
        }
